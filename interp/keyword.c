...
#define MAKE_KEYWORD(key) ksm_itype(KSM_FLAG_KEYWORD, (int)(key), 0, 0)
...
ksm_t ksm_keyword_arg(ksm_t args, ksm_t key, ksm_t if_not_found)
{
  ...
  while( ksm_is_cons(args) ){
    head = ksm_car(args);
    tail = ksm_cdr(args);
    if( !(IS_KEYWORD(head) && ksm_is_cons(tail)) )
      ksm_error("invalid keyword arg: %k", args);
    if( KEYWORD_KEY(head) == key )
      return ksm_car(tail);
    args = ksm_cdr(tail);
  }
  return if_not_found;
...
{
  ksm_install_marker(KSM_FLAG_KEYWORD, mark_keyword);
  ksm_colon_reader = colon_reader;
  ksm_install_fprintf(KSM_FLAG_KEYWORD, fprintf_keyword);
}
